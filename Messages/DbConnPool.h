...
class DbConnPool
{
    ...
    void connect(int num)
    {
        ...
        for (int i = 0; i < num; i++)
        {
            ...
            dbconns_[i].conn = mysql_real_connect(dbconns_[i].conn, "127.0.0.1", "yc", "654321", dbname_.c_str(), 0, NULL, 0);
    ...
    MYSQL *getConn()
    {
        ...
        conn = mysql_real_connect(conn, "127.0.0.1", "yc", "654321", dbname_.c_str(), 0, NULL, 0);
        if (conn == nullptr)
        {
            printf("mysql_real_connect Failed\n");
            return nullptr;
